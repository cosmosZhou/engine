...
template<typename _CharT>
std::basic_string<_CharT>& str_replace(std::basic_string<_CharT> &text,
		const std::basic_string<_CharT> &old,
		const std::basic_string<_CharT> &NEW) {
	int start = 0;
	for (;;) {
		size_t index = text.find(old, start);
		if (index == -1)
			break;
		text = text.replace(index, old.size(), NEW);
		start = index + NEW.size();
	}
	return text;
}
