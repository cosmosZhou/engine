...
PyObject* Caret::append_literal(const string &infix, int &i, char mark) {
	...
	auto string = getitem(infix, { i, end });

	parent = this->parent;
	auto caret = new Literal(string, parent);
...
PyObject* Caret::append_digit(string &infix, int &i) {
	int end = search_for_digits(infix, i);

	if (end == i) {
		throw std::runtime_error("digits not found!");
	}

	digits = getitem(infix, { i, end });

	parent = this->parent;
	auto caret = new Number(digits, parent);
	parent->replace(this, caret);
...
PyObject* Caret::append_left_parenthesis() {
	...
	auto parenthesis = new Parenthesis(caret, parent);
...
PyObject* Caret::append_left_bracket() {
	...
	func = new ArrayList( { this }, parent);
